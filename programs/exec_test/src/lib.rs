...
fn execute_compression() -> Result<()> {
    let uncompressed = [
        ...
        191, 218, 122, 42, 81, 193, 84,
    ];
    let compressed = [
        45, 206, 255, 166, 152, 55, 128, 138, 79, 217, 145, 164, 25, 74, 120, 234, 234, 217, 68,
        149, 162, 44, 133, 120, 184, 205, 12, 44, 175, 98, 168, 172,
    ];
    sol_log_compute_units();
    let decompressed = alt_bn128_compression_g1_decompress(compressed.as_slice()).unwrap();
    assert_eq!(uncompressed, decompressed);
    let decompressed = alt_bn128_compression_g1_compress(decompressed.as_slice()).unwrap();
    assert_eq!(compressed.to_vec(), decompressed);
    Ok(())
}
...
fn verify() -> Result<()> {
    ...
    let mut verifier =
        Groth16Verifier::new(&proof_a, &proof_b, &proof_c, &PUBLIC_INPUTS, &VERIFYING_KEY).unwrap();
    verifier.verify().unwrap();
    Ok(())
}

fn verify_compressed() -> Result<()> {
    let proof_a = [
        45, 206, 255, 166, 152, 55, 128, 138, 79, 217, 145, 164, 25, 74, 120, 234, 234, 217, 68,
        149, 162, 44, 133, 120, 184, 205, 12, 44, 175, 98, 168, 172, 28, 75, 118, 99, 15, 130, 53,
        222, 36, 99, 235, 81, 5, 165, 98, 197, 197, 182, 144, 40, 212, 105, 169, 142, 72, 96, 177,
    ...
    msg!("compressing g1");
    sol_log_compute_units();
    let compressed = alt_bn128_compression_g1_compress(proof_a.as_slice()).unwrap();
    sol_log_compute_units();
    msg!("decompressing g1");
    sol_log_compute_units();
    let proof_a = alt_bn128_compression_g1_decompress(compressed.as_slice()).unwrap();
    sol_log_compute_units();
    msg!("compressing g2");
    sol_log_compute_units();
    let compressed = alt_bn128_compression_g2_compress(proof_b.as_slice()).unwrap();
    sol_log_compute_units();
    msg!("decompressing g2");
    sol_log_compute_units();
    let proof_b = alt_bn128_compression_g2_decompress(&compressed).unwrap();
    sol_log_compute_units();
    let compressed = alt_bn128_compression_g1_compress(proof_c.as_slice()).unwrap();
    let proof_c = alt_bn128_compression_g1_decompress(&compressed).unwrap();
    let mut verifier =
        Groth16Verifier::new(&proof_a, &proof_b, &proof_c, &PUBLIC_INPUTS, &VERIFYING_KEY).unwrap();
    verifier.verify().unwrap();
    Ok(())
...
#[test]
fn test_compression() {
    execute_compression().unwrap();
}
#[test]
fn test_verifier() {
    verify_compressed().unwrap();
}
